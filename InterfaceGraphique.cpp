...
void InterfaceGraphique::fct_etatInitial()
{
    ...
    M = E.RenvoyerEtatReseauSelonTemps(0,GDF,"RdP.txt");
...
void InterfaceGraphique::fct_avancer()
{
	...
	std::cout << "Te avant Transit = " << M.getTe() << std::endl;
	int *Tab = M.Tirage(GDF);
	M.Activer_Transitions(Tab,GDF);
	int valid = 1;
	int i = 1;
	
	std::cout << "Te après Transit = " << M.getTe() << std::endl;
	std::cout << "validavant = " << valid << std::endl; 
	
	/*if(M.getTe() > 1){
		for(; i < M.getTe() - 1; i++){
			//if(M.getTe() == GDF.rechercheEtat(i,"temp.txt").getTe()){
				std::cout << "G.getTe() = " << GDF.rechercheEtat(i,"temp.txt").getTe();
				//valid = 0;
			}
		//}
	}
	...
	if(valid == 1) {E.AjouterEtatReseau(M,GDF);}
	std::cout << "----------AVANCER TEMPS " << M.getTe() << "------------------------------" << std::endl;
	GDF.afficher(M,GDF);
	echeancierintro->setText(QString("Te = %1 <br/>").arg(M.getTe())+QString("S = %1 <br/>").arg(M.getS())+QString("T = %1 <br/>").arg(M.getT()));
...
void InterfaceGraphique::fct_reculer()
{
	...
	if (M.getTe() > 0) {
		...
		//M = GDF.rechercheEtat(M.getTe(), "temp.txt");//E.RenvoyerEtatReseauSelonTemps(M.getTe()-1, GDF,"temp.txt");
		M.Reculer(E,GDF,"temp.txt");
		std::cout << "----------Reculer TEMPS " << M.getTe() << "------------------------------" << std::endl;
		GDF.afficher(M,GDF);
		echeancierintro->setText(QString("Te = %1 <br/>").arg(M.getTe())+QString("S = %1 <br/>").arg(M.getS())+QString("T = %1 <br/>").arg(M.getT()));
...
void InterfaceGraphique::fct_charger()
{
	...
	if (ok_clicked) {
		...
		if (file) {
			...
			GDF.Charger(name);
			M = GDF.rechercheEtat(0, name);//Probleeeme
			GDF.afficher(M,GDF);
